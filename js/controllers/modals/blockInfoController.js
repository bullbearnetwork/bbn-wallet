...
angular.module('liskApp').controller('blockInfoController', ["$scope", "$http", 'riseAPI', "blockInfo", "userInfo", function ($scope, $http, riseAPI, blockInfo, userInfo) {
    ...
    $scope.previousBlock = function (blockId) {
        $http.get(riseAPI.nodeAddress+"/api/blocks/get?id=" +
        blockId)
            .then(function (resp) {
